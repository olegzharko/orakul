...
export const usePowerOfAttorney = ({ initialData, lineItemId, personId }: Props) => {
  ...
  const onSave = useCallback(async () => {
    if (token) {
      ...
      const { success, message } = await reqClientRepresentative(token, lineItemId, personId, 'PUT', reqData);
      dispatch(
        setModalInfo({
          open: true,
          success,
  ...
  }, [lineItemId, data, dispatch, personId, token]);
