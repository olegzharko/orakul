...
export const useFields = () => {
  ...
  const onSave = useCallback(async () => {
    if (token) {
      ...
      const res = await reqManagerClient(token, lineItemId, personId, 'PUT', bodyData);
  ...
  useEffect(() => {
    if (token) {
      // get CLIENT_DATA
      (async () => {
        const res = await reqManagerClient(token, lineItemId, personId);
