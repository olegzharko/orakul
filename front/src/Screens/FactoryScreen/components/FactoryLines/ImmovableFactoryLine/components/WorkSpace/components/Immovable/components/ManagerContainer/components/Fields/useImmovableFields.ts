...
export const useImmovableFields = () => {
  ...
  const { immovableId, lineItemId } = useParams<{lineItemId: string, immovableId: string}>();
  ...
  const onSave = useCallback(async () => {
    ...
    if (token) {
      const res = await reqManagerImmovables(token, lineItemId, immovableId, 'PUT', bodyData);
      ...
      if (res?.success && immovableId === 'create') {
        history.push(
          routes.factory.lines.immovable.sections.immovables.immovableView.linkTo(
            lineItemId, res?.data.immovable_id
          )
        );
      }
    }
  ...
  useEffect(() => {
    if (token) {
      // get MANAGER_GENERAL
      (async () => {
        const res = await reqManagerImmovables(token, lineItemId, immovableId);
