...
import reqClientName from '../../../../../../../../services/notarize/PowerOfAttorney/Client/reqClientName';
...
export const useFio = ({ initialData, clientType }: Props) => {
  ...
  const onSave = useCallback(async () => {
    if (token) {
      const { success, message } = await reqClientName(token, id, 'PUT', data, clientType);
      dispatch(
        setModalInfo({
          open: true,
          success,
  ...
  }, [data, dispatch, clientType, token]);
